## 1. Preloads带来的循环引用问题
`BUG：当打完boss，重新开始游戏，选择角色后点击start，然后报错：Failed to instantiate scene state of \"%s\", node count is 0. Make sure the PackedScene resource is valid无法进入Run场景。神奇的是，当我运行时加了句print打印日志保存后，再点击开始就正常进入到Run场景了。

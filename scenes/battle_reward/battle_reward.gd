...
const CARD_REWARDS_PANEL = preload("uid://dmw46pyjmwtii")
const REWARD_BUTTON = preload("uid://cvkjc7g4hfwg2")
...
func _show_card_reward() -> void:
	if not run_stats or not character_stats:
		return
	var card_rewards := CARD_REWARDS_PANEL.instantiate() as CardRewards

...
const MAP_ROOM = preload("uid://55lvila7iv85")
const MAP_LINE = preload("uid://mla4h4r23meu")
...
var floors_climbed: int		# 当前所在楼层
var last_room: Room
var camera_edge_v: float	# 相机能够移动的最大边界


func _ready() -> void:
	camera_edge_v = MapGenerator.Y_DIST * (MapGenerator.FLOOR - 1)
	#generate_new_map()
	#unlock_floor(0)
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event.is_action_pressed("scroll_up"):
		...
		camera_2d.position.y += SCROLL_SPEED
	camera_2d.position.y = clamp(camera_2d.position.y, -camera_edge_v, 0)
